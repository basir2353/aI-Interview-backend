...
export async function bootstrapDatabase(): Promise<void> {
  ...
  await runStep('reports', async () => {
    ...
    await query(`ALTER TABLE reports ADD COLUMN IF NOT EXISTS report_status VARCHAR(20) NOT NULL DEFAULT 'draft';`);
    await query(`ALTER TABLE reports ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();`);
  });

  // 6b. Interview responses, sessions, backups, transcription audit
  await runStep('interview_responses_and_sessions', async () => {
    await query(`
    ...
    `);
    await query(`CREATE INDEX IF NOT EXISTS idx_interview_responses_interview ON interview_responses(interview_id);`);
    await query(`CREATE INDEX IF NOT EXISTS idx_interview_responses_turn ON interview_responses(turn_id);`);
    await query(`CREATE INDEX IF NOT EXISTS idx_interview_responses_eval_status ON interview_responses(interview_id, evaluation_status);`);
    ...
    `);
    await query(`CREATE INDEX IF NOT EXISTS idx_interview_sessions_candidate ON interview_sessions(candidate_id);`);
    ...
    `);
    await query(`CREATE INDEX IF NOT EXISTS idx_session_backups_interview ON session_backups(interview_id, created_at DESC);`);
